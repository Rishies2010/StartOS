...
static void mouse_wait(uint8_t type) {
    ...
    while (timeout--) {
        if (inportb(0x64) & (type == 0 ? 0x01 : 0x02)) return;
    }
}
...
void mouse_handler(registers_t* regs) {
    ...
    switch (mouse.cycle) {
        ...
        case 2:
            ...
            mouse.x += dx;
            mouse.y += dy;
            if (mouse.x >= framebuffer_width) mouse.x = framebuffer_width - 1;
            if (mouse.y >= framebuffer_height) mouse.y = framebuffer_height - 1;
            mouse.cycle = 0;
            mouse.ready = true;
            break;
    }
...
uint8_t mouse_button(void) {
    if (mouse.buttons & 0x01) return 1;
    if (mouse.buttons & 0x04) return 2;
    if (mouse.buttons & 0x02) return 3;
    return 0;
}
...
void mouse_set_pos(uint32_t x, uint32_t y) {
    if (x < framebuffer_width) mouse.x = x;
    if (y < framebuffer_height) mouse.y = y;
}